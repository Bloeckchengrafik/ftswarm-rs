...
impl_int_updateable!(Voltmeter);
impl_swarm_object!(Voltmeter, Hysteresis);

impl NewSwarmObject<Hysteresis> for Voltmeter {
    ...
    fn init(&mut self) -> impl Future<Output = ()> {
        async move {
            self.run_command(
                ...
                vec![Argument::SensorType(SensorType::Voltmeter), NormallyOpen::Open.into()]
            ).await.unwrap();
