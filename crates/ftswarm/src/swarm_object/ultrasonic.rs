...
impl_int_updateable!(Ultrasonic);
impl_swarm_object!(Ultrasonic, Hysteresis);

impl NewSwarmObject<Hysteresis> for Ultrasonic {
    ...
    fn init(&mut self) -> impl Future<Output = ()> {
        async move {
            self.run_command(
                ...
                vec![Argument::SensorType(SensorType::Ultrasonic), NormallyOpen::Open.into()]
            ).await.unwrap();
